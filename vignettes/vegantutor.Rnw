...
<<fig=true,echo=false>>=
<<a>>
k <- c(8,25,64,91,124,148,149,171,202)
text(pl, "sp", lab=shnam[k], sel=k, cex=0.7, pos=4)
k <- c(126,136,143,157,158,173,176,181,211)
text(pl, "sp", lab=shnam[k], sel=k, cex=0.7, pos=2)
text(pl, "sp", lab=shnam[c(12,33,80)], sel=c(12,33,80), pos=1, cex=0.7)
text(pl, "sp", lab=shnam[c(1,29)], sel=c(1,29), pos=3, cex=0.7)
@
}
...
methods:
\begin{itemize}
\item Distance-based redundancy analysis (db\textsc{rda}) is related
  to metric scaling and can handle any dissimilarity measures and
  performs a linear mapping.  The \texttt{vegan} package has two
  alternative implementations: \code{dbrda} analyses directly
  dissimilarities and can give negative eigenvalues, and
  \code{capscale} only analyses the real components of the
  dissimilarities.
...
from it.  Function \code{capscale} inherits directly from \code{rda},
and through this from \code{cca}, and \code{dbrda} inherits from
\code{cca}.  Many functions, are common with all these methods, and
there are specific functions only if the method deviates from its
ancestor.  In \texttt{vegan} version
\Sexpr{packageDescription("vegan")$Version} the following class
functions are defined for these methods:
<<echo=false>>=
...
component is three.
The rank of unconstrained component is 20,
...
and they are aliased\index{alias} in the analysis, and an informative
message is printed with the result.

It is very common to calculate the proportion of constrained inertia
from the total \idxtext{inertia}.  In \textsc{rda}\index{redundancy
  analysis} this would be the proportion of variance explained by the
constraints.  This is a conventional indicator of goodness of fit, but
most of the total \idxtext{inertia} may be random noise, and we
explain a low proportion of total inertia although we find a strong
signal and informative results.  It may be better to concentrate on
results instead of these proportions.
...
plot(vare.cca)
@
Now the ordination diagram adds arrows for constraints.  These
...
function \code{ordiplot3d} that shows all arrows in full length:
<<>>=
library(vegan3d)
...
that can be spinned or zoomed interactively with your mouse.
...
like this:
<<echo=false>>=
with(dune.env, contrasts(Management))
@
...
<<echo=false>>=
with(dune.env, contrasts(Moisture))
@
...
\idxtext{permutation tests}: the data are permuted randomly and the model
...
numbers of degrees of freedom, since this ``pseudo-$F$'' has little
...
``pseudo-$F$'' is needed in more complicated models.

The permutations are based on \texttt{permute} package which allows also
powerful restricted permutation models. Please consult the vignette of
the \texttt{permute} package for deeper insight.
...
anova(mod, by = "term")
@
All terms are compared against the same residuals.
The test is sequential, and
the order of terms will influence the results, unless the terms are
uncorrelated. The sum of test statistics (\texttt{ChiSquare}) for
terms is the same as the \texttt{Model} test statistic in the overall
test.

``Type III'' tests analyse the marginal effects when each term in turn is
...
correlated terms will get higher (``worse'') $P$-values.  The
...
We can also ask for a test of individual axes, but you should be very
careful with your inference, and axes after the first one may have too
high $P$-values:
<<>>=
...
constrained ordination is best used with designed \textsl{a priori}
...
automatically select constraints into model using function
\code{ordistep}.  Function works similarly as standard \textsf{R}
function \code{text}: it can start from an empty or small model and
add constraints (\texttt{"forward"}), or it can start from a complete
or large model and remove constraints (\texttt{"backward"}), or it can
alternate between adding and removing constraints
(\texttt{"stepwise"}). The decisions are based on permutation
tests. In \texttt{"stepwise"} model building we usually have a lower
significance level to enter a constraint into model than to remove a
selected variable to avoid eternal cycles. The \code{ordistep}
function also calculates a goodness of fit statistic known as
\idxtext{AIC}. This is calculate similarly as in linear models, but it
in ordination it is not based on solid theory, and should be regarded
as purely descriptive indicator of the goodness of fit.

The \code{ordistep} function may be useful in helping to gain insight
into the data, but it should not be trusted blindly (or at all), but
only regarded as an aid in model building. In addition to
\code{ordistep}, \texttt{vegan} also offers function \code{ordiR2step}
that also uses $R^2$ as a criterion, and even standard \textsf{R}
function \code{step} will work, but it does not base its model
selection on signficance tests, and can give different results than
\code{ordistep}.

Using \code{ordistep} is much simpler than explaining how it works.
We need to give the model we start with, and the scope of possible
models inspected.  For this we need another formula trick:
\idxtext{formula} with only \texttt{1} as the constraint defines an
unconstrained model.  We must define it like this so that we can add
new terms to initially unconstrained model. The \texttt{scope} must be
given as a \texttt{list} a formula, but we can extract this from
fitted models using function \texttt{formula}.  The following
\code{ordistep} begins with an unconstrained model \texttt{mod0} and
steps towards the previously fitted maximum model \texttt{mod1}:
<<echo=false>>=
op <- options(digits=7)
@
<<>>=
...
place).  \code{ordistep} alternated between additions (\texttt{+}) and
removals (\texttt{-}) of variables, and the variables are listed in
the order of \idxtext{AIC}.  The stepping stops when no variables can
be entered in the model nor removed from the model.

Model building with automatic procedures (\code{ordistep},
\code{ordiR2step}, \code{step}) is fragile, and the strategy of model
building can change the final model.  If we start with the largest
model (\texttt{mod1}), the final model will be different:
<<>>=
modb <- ordistep(mod1, trace = FALSE, Pout = 0.05)
modb
@

We set the removal $P$-value limit to \texttt{Pout = 0.05} in
\code{ordistep} so that all retained constraints are significant in
the conventional sense.  We also suppressed tracing to save some pages
of output, but \code{ordistep} adds its history in the result:
<<>>=
modb$anova
@
...
\item Weighted averages scores \textsc{wa}\index{WA scores} which are
  \idxtext{weighted averages} (CCA) or weighted sums of species
  scores.
\end{enumerate}
...
therefore it is better to use scores that are resistant to randomness.
Another point is that \textsc{lc} scores\index{LC
scores} \emph{are} constraints: the scores are dependent only on
...
how we can predict the factor level from community composition.
...
The interpretation is similar as in discriminant analysis: \textsc{lc}
scores\index{LC scores} give the predicted class centroids, and
\textsc{wa} scores\index{WA scores} give the predicted values.  For
distinct classes, there is no overlap among groups.  In general, the
length of \texttt{ordispider} segments is a visual image of
\idxtext{species--environment correlation}.  \marginpar{%
<<fig=true,echo=false>>=
plot(dune.cca, display = c("lc", "wa"), type="p")
ordispider(dune.cca, col="blue")
@
...
projected onto the arrow to predict the value of the variable.  The
arrow starts from the (weighted) mean of the environmental variable,
and the values increase towards the arrow head, and decrease to the
opposite direction. We still should figure out the unit of
change.  Function \code{calibrate} performs this automatically in
\texttt{vegan}.  Let us inspect the result of the \code{ordistep}
function with three constraints:
<<>>=
pred <- calibrate(mod)
head(pred)
@

The calculation is not actually based on biplot arrows, but on
regression coefficients used internally in constrained ordination.
Biplot arrows should only be seen as a visual approximation.  The
fitting is done in full constrained rank as default and for all
constraints simultaneously.  The example draws a residual plot of
predictions:
<<a>>=
...
\subsection{Partial models with conditions}
\label{sec:pcca}
...
after removing the effects of conditions.  These conditioning
...
variables. However, we do not have strictly random variables in
ordination, but all variables are fitted in the same way.
...
but the groups of environmental variables should be independent for
...
become negative.  Higher-order dependencies are almost certain to
appear with high number of variables and high number of groups.
However, \code{varpart} performs decomposition of \code{rda} or
\code{dbrda} models among two to four components.
...
variation into ordination space, and mainly inspect the first
...
method in \texttt{vegan} is \code{adonis2} which implements a
multivariate analysis of variances using community dissimilarity
matrices. The \code{adonis2} is similar to
\idxtext{distance-based redundancey analysis} (\code{dbdra}):
it is based on the same
method, but skips actual ordination and only finds the goodness-of-fit
statistics.  Function \code{adonis2} can handle both continuous and
factor predictors. Other methods in \texttt{vegan} include
multiresponse permutation procedure (\code{mrpp}) and analysis of
similarities (\code{anosim}). Both of these handle only class
predictors, and they are less robust than \code{adonis2}.

\subsection{adonis: Multivariate ANOVA based on dissimilarities}

Function \code{adonis2} partitions dissimilarities for the sources of
variation, and uses permutation tests to inspect the significances of
those partitions. The results are similar as in \code{dbrda} and its
\code{anova} permutation tests, and with Euclidean
distances\index{dissimilarity!Euclidean} both are equal to \code{rda}.

The example uses \code{adonis2} to study \idxtext{beta diversity}
...
species $S$ is dependent on the area (size) $X$ of the study area.
\marginpar{
\begin{align*}
S =& k X^z\\
\begin{split}
...
@
Function \code{adonis2} can use formula interface, and the dependent
data can be either dissimilarities or data frame, and in the latter
case \code{adonis2} uses \code{vegdist} to find the dissimilarities.
<<>>=
adonis2(betad ~ Management, dune.env)
@
The models can be more complicated, and sequential or marginal tests of
permutational ANOVA\index{permutation tests} can be performed if there are
several parameters:
<<>>=
adonis2(betad ~ A1*Management, dune.env)
@

\subsection{Homogeneity of groups and beta diversity}

Function \code{adonis2} studied the differences in the group means,
but function \code{betadisper} studies the differences in group
homogeneities, or variation within groups. Function \code{adonis2} was
analogous to multivariate analysis of variance, and \code{betadisper}
is analogous to \idxtext{Levene's test} of the equality of variances.

The example continues the analysis of the previous section and
inspects the \idxtext{beta diversity}. The function can only use one
factor as an independent variable, and it does not know the formula
interface, so that we need to use \code{with} to make the factor
...
the correlation between dissimilarity matrices.  As there are
$N(N-1)/2$ dissimilarities among $N$ objects, normal
significance tests are not applicable.  Mantel developed asymptotic
test statistics, but \texttt{vegan} function \code{mantel} uses
...
\code{mantel}.  We first perform a
\textsc{pca}\index{principal components analysis} of environmental
...
\code{scores} in \texttt{vegan} will work with all these methods.
The following uses the same standardizations for community
dissimilarities as previously used in \code{metaMDS}.
<<>>=
...
Procrustes correlation $r$ derived from the symmetric Procrustes
residual $m^2$.
\marginpar{%
<<fig=true,echo=false>>=
plot(pro)
...
The \texttt{vegan} is mainly a package for ordination and diversity
...
\code{hclust}.  Function \code{hclust} needs dissimilarities
as input.
...
to \idxtext{minimum spanning tree} which nicely can be represented in
...
If we wish, we may use all normal statistical methods with factors,
such as functions \code{lm} or \code{aov} for formal testing of
``significance'' of clusters.  Classification can be compared against
external factor variables as well.  However, \texttt{vegan} does not
provide any tools for this.  It may be best to see the
\indexrev{labdsv}{package} and its tutorial for this purpose.
\marginpar{%
<<fig=true,echo=false>>=
<<a>>
@
...
A common reason to classify data is to make a classified community
table.  For this purpose, both sites and species should be arranged so
that the table is structured.  The original clustering may not be
...
The ordering of leaves is arbitrary in the cluster dendrogram: You can
take any branch and rotate it about its base. This changes the order
of leaves, but the tree is topologically identical. We can re-order
tree so that the order of leaves is as similar as possible to an
external variabe. \texttt{Vegan} has function \code{reorder} to change
the order of leaves to mach an external variable, and base \textsf{R}
has similar \code{reorder} function for an alternative
\code{dendrogram} mode of trees. The only continuous
variable in the Dune data is the thickness of A1 horizon, and this
could be used to arrange the tree.  However, for a nicely structured
community table we use another trick: \textsc{ca}\index{correspondence
  analysis} is an ordination method that structures table optimally
into a diagonal structure, and we can use its first axis to reorder
the tree:
<<>>=
...
The classified community tables can be displayed graphically using
\code{tabasco} function or printed in compact text form with
\code{vegemite} in \texttt{vegan}.  These functions take an argument
\texttt{use} to arrange the sites (and species, if possible), and
\code{tabasco} can also display \code{dendrogram}.
\marginpar{%
<<fig=true,echo=false>>=
tabasco(dune, oden)
@
...
This argument can
be a vector used to arrange sites, or it can be an ordination result,
or it can be an \code{hclust} result or a \code{dendrogram} object.
