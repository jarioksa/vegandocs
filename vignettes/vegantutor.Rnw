...
\code{isoMDS} function in the \texttt{MASS}
package\index{package!MASS}.  This function needs dissimilarities as
...
@ 
The default is to find two dimensions and use \idxtext{metric
scaling} (\code{cmdscale}) as the starting solution.  The solution
is iterative, as can be seen from the tracing information (this can be
suppressed setting \texttt{trace = F}).

The results of \code{isoMDS} is a list (items
\texttt{\Sexpr{paste(names(vare.mds0), collapse=", ")}}) for the
...
mapping using function \code{Shepard} in
\texttt{MASS}\index{package!MASS} package, or a simple wrapper
\code{stressplot} in \texttt{vegan}:
<<a>>=
...
correlation based on stress is $R^2 = 1 - S^2$.  The ``fit-based
...
both based on the residuals in the Shepard plot, but they differ in
...
observations are put in the same point, which is geometrically
...
\idxtext{local optimum} instead of finding the global optimum.
...
in \code{isoMDS}\index{non-metric multidimensional scaling} results:
...
\item Function run \code{isoMDS} with several random starts, and
  stopped either after a certain number of tries, or after finding two
  similar configurations with minimum stress.  In any case, it
...
differences and strongly dominated by single large differences.  Most
ecologically meaningful dissimilarities are of Manhattan
...
Package \texttt{vegan} has function \code{vegdist} with
...
All these are of the Manhattan type and use only first order terms
...
between two compared communities.  Function \code{vegdist} is a
drop-in replacement for standard \textsf{R} function \code{dist},
and either of these functions can be used in analyses of
dissimilarities.
...
Raup--Cric,\index{dissimilarity!Raup-Crick}
...
In this case the differences were fairly small, and mainly concerned
two points.  You can use \code{identify} function to identify those
...
variance, and a more scale-independent and symmetric statistic is
found (often known as Procrustes $m^2$).
...
could be used and dissimilarities were nonlinearly mapped into
...
\bottomrule
\end{tabular}
} If we accept only certain types of dissimilarities and make a linear
mapping, the ordination becomes a simple task of rotation and
projection.  In that case we can use eigenvector methods.  Principal
components analysis\index{principal components analysis}
...
first axis ``explains'' \Sexpr{round(vare.pca$CA$eig[1], 1)}/
\Sexpr{round(vare.pca$tot.chi)} $=$
...
For this graph we specified \texttt{scaling = -1}. The results are
scaled only when they are accessed, and we can flexibly change the
scaling in \texttt{plot}, \code{biplot} and other commands. The
negative values mean that species scores are divided by the species
standard deviations so that abundant and scarce species will be
approximately as far away from the origin.
...
\texttt{plot} by setting negative scaling).  Standardizing all species
...
original programme returns as ``eigenvalues'' --- I have no idea of
their possible meaning, and they should not be used.  Most often
people comment on axis lengths, which sometimes are called ``gradient
lengths''.  The etymology is obscure: these are not gradients, but
...
components analysis} should be used.  This is only folklore and not
...
ordination.  However, it seems to be a fragile and vague back of
tricks that is better avoided.
...
\item The variables in data frames are not visible to \textsf{R}
  session unless the data frame is \code{attach}ed to the session.
  We may not want to make all variables visible to the session,
  because there may be synonymous names, and we may use wrong
  variables with the same name in some analyses.  We can use function
  \code{with} which makes the given data frame visible only to the
  following command.
\end{itemize}
...
methods:
\begin{itemize}
\item Constrained analysis of proximities\index{constrained analysis
of proximities} (\textsc{cap}) in function
  \code{capscale} is related to \idxtext{metric scaling}
  (\code{cmdscale}).  It can handle any dissimilarity measures and
  performs a linear mapping.
